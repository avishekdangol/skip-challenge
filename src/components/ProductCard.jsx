...
function ProductCard({ product, selected, onSelect }) {
  ...
  return (
    <>
      {/* Card Container */}
      <div
        className={`border border-gray-400 shadow rounded transition-all duration-200 cursor-pointer relative overflow-hidden ${selected ? 'opacity-75 border-yellow-700 border-3' : 'hover:bg-indigo-950 hover:text hover:border-yellow-500 hover:shadow-md hover:shadow-yellow-500'}`}
        onClick={onSelect}
      >
        {/* Image */}
          ...
          <div className="flex gap-2">
            {/* Not Allowed on Road Badge */}
            {
              !product.allowed_on_road && (
                <div 
                  ...
                  onClick={(e) => { e.stopPropagation(); setOnRoadWarningHovered(!onRoadWarningHovered); }}
            ...
            {/* Heavy Waste Badge */}
            {
              !product.allows_heavy_waste && (
                <div 
                  className="flex text-white items-center bg-yellow-600 px-2 py-2 rounded shadow-lg"
                  onClick={(e) => { e.stopPropagation(); setHeavyWasteWarningHovered(!heavyWasteWarningHovered); }}
            ...
            {/* Forbidden Badge */}
            {
              product.forbidden && (
                <div 
                  ...
                  onClick={(e) => { e.stopPropagation(); setForbiddenHovered(!forbiddenHovered); }}
